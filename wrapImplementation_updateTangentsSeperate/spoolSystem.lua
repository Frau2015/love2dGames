...
function getIntersected(outPos,inPos,ignoreA,ignoreB,spools)
	for _, spool in pairs(spools) do
		...
		-- if spool ~= ignoreA and spool ~= ignoreB and distSqr(spool,ignoreA) > spool.rad^2+ignoreA.rad^2 and distSqr(spool,ignoreB) > spool.rad^2+ignoreB.rad^2 and (not spool.isAttached) and lineCircleIntersect(outPos,inPos,spool) then
